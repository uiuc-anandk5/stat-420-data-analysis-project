...
However, we think our final model could still be useful for predicting a range for the number of subscribers for a MOOC course. It could also be used to predict the general popularity level of the course, which would be helpful to the Udemy platform since they might just want to know which courses to advertise. Additionally, our research altogether could be used as a launching point for further analysis to derive a better model using additional techniques beyond the scope of our class. For instance, there could be other transformations that we could have applied. In addition, our model might have been significantly improved if we had a dataset with additional features, like number of "views" of the course landing page in the last 30 days. 

All in all, we're very content with our model's results, and we're confident that if we were armed with more techniques and a wider range of features, we could have done an even better job predicting Udemy course popularity.
