...
#udemy_log_mod = lm(log1p(num_subscribers) ~ is_paid + log1p(price) + log1p(num_reviews) + num_lectures + level + log(content_duration) + subject + log(days),
#                   data = udemy)
udemy_log_mod = lm(log1p(num_subscribers) ~ is_paid + log1p(price) + log1p(num_reviews) + log(num_lectures) + level + log(content_duration) + subject + log(days) + log(num_lectures):log(content_duration) + is_paid:log1p(price), data = udemy)


summary(udemy_log_mod)
udemy_mod_back_aic = step(udemy_log_mod, direction = "both", trace = 0, 
                          scope = log1p(num_subscribers) ~ is_paid + log1p(price) + log1p(num_reviews) + log(num_lectures) + level + log(content_duration) + subject + log(days) + log(num_lectures):log(content_duration) + is_paid:log1p(price))
udemy_mod_back_bic = step(udemy_log_mod, direction = "both", trace = 0, k = log(n),
                          scope = log1p(num_subscribers) ~ is_paid + log1p(price) + log1p(num_reviews) + log(num_lectures) + level + log(content_duration) + subject + log(days) + log(num_lectures):log(content_duration) + is_paid:log1p(price))
...
```{r}
# checking metrics on log models
get_loocv_rmse(udemy_log_mod)
get_adj_r2(udemy_log_mod)
bptest(udemy_log_mod)$p.value
get_bp_decision(udemy_log_mod, alpha = 0.01)
shapiro.test(resid(udemy_log_mod))$p.value
get_sw_decision(udemy_log_mod, alpha = 0.01)
get_num_params(udemy_log_mod)

get_loocv_rmse(udemy_mod_back_aic)
get_adj_r2(udemy_mod_back_aic)
bptest(udemy_mod_back_aic)$p.value
get_bp_decision(udemy_mod_back_aic, alpha = 0.01)
shapiro.test(resid(udemy_mod_back_aic))$p.value
get_sw_decision(udemy_mod_back_aic, alpha = 0.01)
get_num_params(udemy_mod_back_aic)

get_loocv_rmse(udemy_mod_back_bic)
get_adj_r2(udemy_mod_back_bic)
bptest(udemy_mod_back_bic)$p.value
get_bp_decision(udemy_mod_back_bic, alpha = 0.01)
shapiro.test(resid(udemy_mod_back_bic))$p.value
get_sw_decision(udemy_mod_back_bic, alpha = 0.01)
get_num_params(udemy_mod_back_bic)
```
