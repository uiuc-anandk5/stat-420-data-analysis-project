...
scope3 = log1p(num_subscribers) ~ is_paid + log1p(price) + log1p(num_reviews) + log(num_lectures) + level + log(content_duration) + subject + log(days) + log(num_lectures):log(content_duration)
...
get_rmse = function(model, log){
  preds = ifelse(log == TRUE, expm1(predict(model, newdata = udemy_test)), predict(model, newdata = udemy_test))
...
get_mape = function(model, log){
  preds = ifelse(log == TRUE, expm1(predict(model, newdata = udemy_test)), predict(model, newdata = udemy_test))
  mean(abs((preds - udemy_test$num_subscribers) / preds) * 100)
}
