...
scope1 = log1p(num_subscribers) ~ I(log1p(price) ^ 2) + log1p(num_reviews) + log(content_duration) + days + subject + level
...
criterion = data.frame(
  ...
  'RMSE' = c(expm1(model1$results$RMSE),
             model2$results$RMSE),
  'BP Test' = c(bptest(model1$finalModel)$p.value,
                bptest(model2$finalModel)$p.value),
  'Shapiro Test' = c(shapiro.test(resid(model1$finalModel))$p.value,
